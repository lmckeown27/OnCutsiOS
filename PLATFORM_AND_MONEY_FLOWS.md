# Intera / OnCuts — Platform overview, money flows, and charges
...
**Intera** (this repo’s iOS app) is the consumer-facing client for **OnCuts** — a **campus marketplace for personal services** (primarily barbers / service providers). Students or guests can:
...
The **OnCuts** backend (`OnCutsPackage/backend`) is an **Express** API with **PostgreSQL**, **Stripe** (payments + **Stripe Connect** for providers), webhooks, and optional paths for legacy / experimental wallet features. The mobile app talks to **`/api/v1`** (see `AppConfiguration`).
...
| **Platform (OnCuts)** | Operates the marketplace, sets **take rate** on the service portion of card payments, and holds the **Stripe platform account** used for PaymentIntents. |
...
### 4.2 Charges / revenue the **platform** (OnCuts) receives
...
- `OnCutsPackage/backend/src/routes/booking-simple.routes.ts` (create-payment-intent),
- `OnCutsPackage/backend/src/services/stripe-payment.service.ts`,
- `OnCutsPackage/backend/src/controllers/stripe-webhook-secure.controller.ts`,
- and this file so support and finance stay aligned.

**Last reviewed against repository:** February 2026 (code search across `OnCutsPackage/backend` and `Intera/Intera` payment flows).
