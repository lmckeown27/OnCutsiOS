...
enum AppConfiguration: Sendable {
    // MARK: - API v1 root (OnCuts Express on EC2)
    ...
    /// Barbers list uses this root + `/barbers` (see `urlOnCutsBarbers`).
    private static let onCutsAPIv1BaseURLString = apiV1RootURLString
    ...
    static let onCutsDefaultCampusId: String? = nil

    /// `GET …/api/v1/barbers/:id` — production embeds `reviews` on this payload (Prisma/bookings-backed).
    static func urlOnCutsBarber(barberId: String) -> URL? {
    ...
    static func urlOnCutsBarbers(latitude: Double?, longitude: Double?, maxDistanceKm: Double? = nil) -> URL {
        var components = URLComponents(string: onCutsAPIv1BaseURLString + "/barbers")!
        var items: [URLQueryItem] = []
        if let cid = onCutsDefaultCampusId?.trimmingCharacters(in: .whitespacesAndNewlines), !cid.isEmpty {
    ...
    /// Same path as `urlOnCutsBarbers(latitude:nil, longitude:nil)` — no geo or campus query (unless `onCutsDefaultCampusId` is set).
    static var urlOnCutsBarbers: URL {
        urlOnCutsBarbers(latitude: nil, longitude: nil, maxDistanceKm: nil)
    }
    ...
    /// GET current user (Bearer) — includes `needsPlatformPassword` / `needs_platform_password` per OnCuts contract.
    ...
    /// Legacy list URL; home screen tries `urlOnCutsBarbers` first, then this.
