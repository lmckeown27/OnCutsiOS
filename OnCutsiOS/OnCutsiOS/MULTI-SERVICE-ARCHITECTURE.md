...
This document outlines how the Intera app supports multiple service platforms (OnCuts for haircuts, BeautyPlatform for beauty services, etc.) with each pulling from different packages/backends.
...
**Files**:
- `OnCutsAdapter.swift` - Converts `Barber` → `ServiceProvider`
...
enum ServicePlatform: String, CaseIterable {
    case onCuts = "haircuts"
    case beautyPlatform = "beauty"
    case wellness = "wellness"
    
    var displayName: String {
        switch self {
        case .onCuts: return "Haircuts"
    ...
    var packageName: String {
        switch self {
        case .onCuts: return "OnCutsModule"
...
class OnCutsService: ServiceProviderSource {
    let providerType: ServicePlatform = .onCuts
...
1. User taps "Haircuts" in ServiceSelectionScreen
   ↓
2. MainCoordinator sets selectedService = .onCuts
   ↓
3. ConsumerHomeScreen loads
   ↓
4. ServiceRegistry returns OnCutsService
   ↓
5. OnCutsService.fetchProviders() called
   ↓
6. OnCuts API returns [Barber]
   ↓
...
│   │
│   └── Adapters/
│       ├── OnCutsAdapter.swift       # Barber → ServiceProvider
│       └── BeautyPlatformAdapter.swift   # BeautySpecialist → ServiceProvider
│
├── Packages/
│   ├── OnCutsModule/
│   │   ├── Models/
│   │   │   └── Barber.swift              # OnCuts-specific model
│   │   ├── Services/
│   │   │   └── OnCutsAPI.swift
│   │   └── OnCutsService.swift
│   │
...
- Can update OnCuts without affecting Beauty platform
...
- OnCuts can use AWS Cognito + Stripe
...
- [ ] Create `OnCutsAdapter` to convert `Barber` → `ServiceProvider`
...
- [ ] Move OnCuts-specific code to module
...
extension ServiceProvider {
    static let onCutsMocks = Barber.mocks.map { $0.toServiceProvider() }
    static let beautyMocks = BeautySpecialist.mocks.map { $0.toServiceProvider() }
    static let allMocks = onCutsMocks + beautyMocks
}
```

## Next Steps

1. **Keep existing `Barber.swift`** - This is from OnCuts package
