...
struct LiveBookingView: View {
    ...
    @State private var packageServiceRows: [OnCutsBarberServiceRow] = []
    ...
    private var onCutsBarberId: String {
    ...
    private func loadPackageServices() async {
        guard !onCutsBarberId.isEmpty else {
            packageServiceRows = []
            return
        }
        do {
            packageServiceRows = try await onCutsClient.fetchBarberServiceRows(barberId: onCutsBarberId)
        } catch {
            packageServiceRows = []
        }
    }
