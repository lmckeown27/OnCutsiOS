...
//  Terms: inline `OnCutsTermsOfServiceDocumentScrollView` + agreement on the same screen. Keychain resume may still show a post-verify Terms card when `pendingSession` is set.
//
...
final class SignupCoordinator {
    ...
    var pendingSession: OnCutsAuthSession?
    ...
    /// US 10-digit mobile → E.164 `+1…` (matches web + OnCuts backend defaults).
    ...
    /// Matches `syntheticEmailFromPhoneE164` in the OnCuts API.
    ...
    func sendPhoneVerificationAfterContact() async {
        ...
        defer { isSubmitting = false }
        do {
            let sent = try await OnCutsAuthService.sendPhoneSignupCode(
    ...
    func submitPasswordAndSendVerification() async {
        ...
        do {
            let sent = try await OnCutsAuthService.sendVerificationCode(apiV1BaseTrimmed: apiBase, request: req)
            email = sent.email
            devVerificationHint = sent.devVerificationCode
            verificationCode = ""
            errorMessage = nil
    ...
    func resendVerificationEmail() async {
        ...
        defer { isSubmitting = false }
        do {
            let sent = try await OnCutsAuthService.resendVerificationCode(email: em, apiV1BaseTrimmed: apiBase)
            email = sent.email
            devVerificationHint = sent.devVerificationCode
            verificationCode = ""
            SignupOnboardingPersistence.save(step: .verification, email: email)
    ...
    func performVerify(sessionManager: AppSessionManager) async {
        ...
        defer { isSubmitting = false }
        do {
            var session = try await OnCutsAuthService.verify(code: code, email: em, apiV1BaseTrimmed: apiBase)
            ...
            OnCutsLiquidGlassHaptics.notification(.success)
            #endif
            sessionManager.login(session: UserSession(onCutsVerified: session))
            SignupOnboardingPersistence.clearAll()
            pendingSession = nil
            ...
            verificationCode = ""
            #if os(iOS)
            OnCutsLiquidGlassHaptics.notification(.error)
            #endif
    ...
    private func verifyPhoneCodeThenPassword() async {
        ...
        defer { isSubmitting = false }
        do {
            let result = try await OnCutsAuthService.verifyPhoneSignupCode(
                apiV1BaseTrimmed: apiBase,
                phone: phoneE164,
                code: code
            )
    ...
    func submitPhonePasswordAndCompleteSignup(sessionManager: AppSessionManager) async {
        ...
        defer { isSubmitting = false }
        do {
            var session = try await OnCutsAuthService.completePhoneSignup(
                apiV1BaseTrimmed: apiBase,
    ...
    func finishRestoredVerifiedUserIfNeeded(sessionManager: AppSessionManager) {
        ...
        sessionManager.login(session: UserSession(onCutsVerified: s))
        pendingSession = nil
...
struct LiquidGlassSignupFlowView: View {
    ...
    /// Keychain resume: verified session not yet applied — show the legacy “Enter OnCuts” Terms card, not the pre-account Terms step.
    ...
    var body: some View {
        ...
        .onChange(of: coordinator.errorMessage) { _, new in
            ...
            let p = OnCutsAuthUserMessaging.signUpAlertPresentation(message: new)
    ...
    private var stepHeader: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            if !headerTitle.isEmpty {
                Text(headerTitle)
                    .font(OnCutsLiquidGlassTypography.title(28, weight: .semibold))
                        ...
                        glassTextField(title: "Mobile number", content: {
                            HStack(alignment: .center, spacing: 12) {
                                ...
                                VStack(alignment: .leading, spacing: 8) {
                                    ...
                                    #endif
                                    .font(OnCutsFont.title2.monospacedDigit().weight(.semibold))
                                    ...
                                    HStack {
                                        Text("Digits only · area + number")
                                            .font(OnCutsFont.caption2(weight: .medium))
                                            .foregroundStyle(.tertiary)
                                        Spacer(minLength: 0)
                                        Text("\(min(coordinator.phoneDigits.count, 10))/10")
                                            .font(OnCutsFont.caption2.monospacedDigit().weight(.semibold))
                                            .foregroundStyle(
        ...
        liquidCard {
            VStack(spacing: 18) {
                ...
                if let hint = coordinator.devVerificationHint, !hint.isEmpty {
                    Text("Dev code: \(hint)")
                        .font(OnCutsFont.caption.monospaced())
                        .foregroundStyle(.tertiary)
                        .frame(maxWidth: .infinity, alignment: .leading)
                }
                ...
                ZStack {
                    ...
                    TextField("", text: otpBinding)
                        ...
                        .autocorrectionDisabled()
                        .font(OnCutsFont.body.monospacedDigit())
        ...
        liquidCard {
            VStack(spacing: 20) {
                Text("Read before you continue")
                    .font(OnCutsLiquidGlassTypography.title(24, weight: .bold))
                    .multilineTextAlignment(.center)
        ...
        liquidCard {
            VStack(spacing: 20) {
                Image(systemName: "sparkles")
                    .font(OnCutsFont.system(size: 48))
    ...
    private func enterOnCutsFromComplete() {
