...
//  Never uses the Google ID token as the OnCuts API Bearer (that caused 401 on messaging).
//
...
struct SessionAuthTokens: Sendable {
    ...
    /// OnCuts `users.id` when returned by auth (required for APIs keyed by DB user id).
...
enum AuthBackendVerification {
    ...
    /// POST JSON `{ "idToken": "<Google JWT>" }` and return OnCuts access (and optional refresh) tokens.
