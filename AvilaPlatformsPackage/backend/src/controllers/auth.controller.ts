 ...
 * {
 *   "success": true,
 *   "message": "Email verified successfully. Welcome to AvilaPlatforms!",
 *   "data": {
 *     "user": {
  ...
  try {
    ...
    res.status(201).json({
      ...
      message: hasApprovedApplication 
        ? 'Email verified successfully. Welcome to AvilaPlatforms! Your barber application has been linked to your account.'
        : 'Email verified successfully. Welcome to AvilaPlatforms!',
      data: {
        user: {
          id: user.id,
          email: user.email,
...
/**
 * POST /api/v1/auth/google — Intera (and other clients): exchange Google `id_token` for AvilaPlatforms JWTs.
...
/**
 * POST /api/v1/auth/apple — Exchange Apple `identityToken` (JWT) for AvilaPlatforms JWTs (App Store Guideline 4.8).
