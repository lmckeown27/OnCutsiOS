//
//  AvilaPlatformsModuleTests.swift
//  AvilaPlatformsModule
//

import XCTest
@testable import AvilaPlatformsModule

final class AvilaPlatformsModuleTests: XCTestCase {
    ...
    func testModuleBuilderCreatesViews() async {
        ...
        await MainActor.run {
            let _ = AvilaPlatformsModuleBuilder.build(with: mockSession)
            let _ = AvilaPlatformsModuleBuilder.buildConsumerView(with: mockSession)
            let _ = AvilaPlatformsModuleBuilder.buildBarberDashboard(with: mockSession)
            let _ = AvilaPlatformsModuleBuilder.buildRoleBasedView(with: mockSession)
        }
    }
}
