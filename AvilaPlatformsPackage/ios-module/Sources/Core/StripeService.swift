...
public enum StripeService {
    ...
    /// AvilaPlatforms, a bundled `pk_test_…` or an unresolved `$(STRIPE_PUBLISHABLE_KEY)` is **not** applied, so the SDK
    ...
    public static func applyPublishableKeyAlignedWithAPIHost(apiRootTrimmed: String, bundle: Bundle = .main) {
        ...
        let isProductionAvilaPlatforms = root.lowercased().contains("campuscut.com")
