...
public enum AvilaPlatformsSignUpAPIError: LocalizedError, Sendable {
    case invalidURL
...
/// Minimal REST client for AvilaPlatforms email registration + verification (matches `auth.controller.ts`).
public enum AvilaPlatformsSignUpAPI {
    public static func fetchCampuses(apiV1BaseTrimmed: String) async throws -> [AvilaPlatformsSignUpCampus] {
        let base = apiV1BaseTrimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        guard let url = URL(string: base + "/campus") else {
            throw AvilaPlatformsSignUpAPIError.invalidURL
        }
        let (data, http) = try await dataGET(url: url)
        guard (200 ... 299).contains(http.statusCode) else {
            throw AvilaPlatformsSignUpAPIError.httpStatus(http.statusCode, String(data: data, encoding: .utf8))
        }
        ...
        guard let envelope = try? decoder.decode(CampusesEnvelope.self, from: data),
            ...
            throw AvilaPlatformsSignUpAPIError.decodingFailed
        }
        return rows.compactMap { row in
            let id = row.id?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            let name = row.name?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
            guard !id.isEmpty, !name.isEmpty else { return nil }
            return AvilaPlatformsSignUpCampus(id: id, name: name)
        }
    }

    public static func register(
        apiV1BaseTrimmed: String,
        request: AvilaPlatformsRegisterRequest
    ) async throws -> AvilaPlatformsRegisterSentResult {
        let base = apiV1BaseTrimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        ...
        guard let http = resp as? HTTPURLResponse else {
            throw AvilaPlatformsSignUpAPIError.invalidResponse
        }
        guard (200 ... 299).contains(http.statusCode) else {
            throw AvilaPlatformsSignUpAPIError.httpStatus(http.statusCode, serverMessage(from: data))
        }
        let decoder = JSONDecoder()
        decoder.keyDecodingStrategy = .convertFromSnakeCase
        ...
        return AvilaPlatformsRegisterSentResult(email: emailResolved, phone: phoneResolved, devVerificationCode: code)
    }
    ...
    public static func resendVerification(
        ...
        if let p = phone?.trimmingCharacters(in: .whitespacesAndNewlines), !p.isEmpty {
            ...
            throw AvilaPlatformsSignUpAPIError.missingEmailOrPhone
        }
        ...
        let code = block.verificationCode?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty
        return AvilaPlatformsRegisterSentResult(email: em, phone: phoneOut, devVerificationCode: code)
    }
    ...
    public static func verifyEmail(
        ...
        phone: String?,
        code: String
    ) async throws -> AvilaPlatformsVerifiedSession {
        let base = apiV1BaseTrimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
    ...
    public static func sendPhoneSignupCode(
        ...
        role: String,
        campusId: String?
    ) async throws -> AvilaPlatformsPhoneSignupSendResult {
        let base = apiV1BaseTrimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        ...
        let code = block.verificationCode?.trimmingCharacters(in: .whitespacesAndNewlines).nilIfEmpty
        return AvilaPlatformsPhoneSignupSendResult(phone: phoneOut, devVerificationCode: code)
    }
    ...
    public static func verifyPhoneSignupCode(
        ...
        phone: String,
        code: String
    ) async throws -> AvilaPlatformsPhoneSignupVerifyResult {
        let base = apiV1BaseTrimmed.trimmingCharacters(in: CharacterSet(charactersIn: "/"))
        ...
        let phoneOut = block.phone?.trimmingCharacters(in: .whitespacesAndNewlines) ?? phone
        return AvilaPlatformsPhoneSignupVerifyResult(phoneSignupToken: tok, phone: phoneOut)
    }
    ...
    private static func decodeVerifiedSession(from data: Data, fallbackEmail: String) throws -> AvilaPlatformsVerifiedSession {
        let decoder = JSONDecoder()
        decoder.keyDecodingStrategy = .convertFromSnakeCase
        ...
        return AvilaPlatformsVerifiedSession(
            accessToken: access,
