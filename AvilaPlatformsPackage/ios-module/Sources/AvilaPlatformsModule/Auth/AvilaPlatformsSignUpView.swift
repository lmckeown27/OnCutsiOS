...
/// Email + password sign-up aligned with the legacy AvilaPlatforms iOS `SignUpView` and `POST /api/v1/auth/register` + `verify-email`.
public struct AvilaPlatformsSignUpView: View {
    public let apiV1BaseTrimmed: String
    /// Called after email verification returns JWTs.
    public let onSignedIn: (AvilaPlatformsVerifiedSession) -> Void
    ...
    @State private var selectedCampus: AvilaPlatformsSignUpCampus?
    @State private var campuses: [AvilaPlatformsSignUpCampus] = []
    ...
    public init(apiV1BaseTrimmed: String, onSignedIn: @escaping (AvilaPlatformsVerifiedSession) -> Void) {
    ...
    private func loadCampusesIfNeeded() async {
        ...
        do {
            campuses = try await AvilaPlatformsSignUpAPI.fetchCampuses(apiV1BaseTrimmed: apiV1BaseTrimmed)
        } catch {
            campuses = []
        }
    }
    ...
    private func submitRegister() async {
        ...
        do {
            let sent = try await AvilaPlatformsSignUpAPI.register(apiV1BaseTrimmed: apiV1BaseTrimmed, request: req)
    ...
    private func submitVerify() async {
        ...
        do {
            let session = try await AvilaPlatformsSignUpAPI.verifyEmail(
