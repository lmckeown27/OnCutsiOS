# AvilaPlatforms iOS Module

A Swift Package feature module for the AvilaPlatforms platform, designed to be pulled into a Shell app using a **Contract-First** architecture.
...
│       ├── AvilaPlatformsModuleBuilder.swift    # Public entry factory
│       ├── Protocols/
...
4. Add `AvilaPlatformsModule` to your target

### Or via Package.swift

```swift
dependencies: [
    .package(url: "https://github.com/your-org/AvilaPlatforms.git", from: "1.0.0")
    // Or for subdirectory:
    .package(path: "../AvilaPlatforms/ios-module")
]
```
...
Use the `AvilaPlatformsModuleBuilder` to get the entry view:
...
struct ContentView: View {
    ...
    var body: some View {
        ...
        AvilaPlatformsModuleBuilder.buildRoleBasedView(with: session)
        
        // Option B: Explicitly choose the view
        // AvilaPlatformsModuleBuilder.buildConsumerView(with: session)
        // AvilaPlatformsModuleBuilder.buildBarberDashboard(with: session)
    }
}
```
...
NavigationLink(destination: AvilaPlatformsModuleBuilder.build(with: session)) {
    ServiceTile(
        icon: "scissors",
...
|--------------|------------|
| **Public** | `AvilaPlatformsModuleBuilder`, `UserSessionProtocol` |
...
The module connects to the AvilaPlatforms backend API at `https://api.campuscut.com/api/v1`. It uses the injected `accessToken` for all authenticated requests.
...
- [AvilaPlatforms Web App](../web-app) - React/Vite frontend
- [AvilaPlatforms Backend](../backend) - Node.js/Express API
