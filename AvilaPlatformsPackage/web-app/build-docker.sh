...
# AvilaPlatforms Frontend - Docker Build Script
# Builds the Docker image with proper environment variables

set -e  # Exit on error

echo "🐳 Building AvilaPlatforms Frontend Docker Image..."
echo ""
...
export VITE_APP_NAME=${VITE_APP_NAME:-AvilaPlatforms}
...
docker build \
  ...
  -t avilaplatforms-frontend:latest \
  -t avilaplatforms-frontend:${VITE_APP_VERSION} \
  .
...
echo "   docker run -d -p 80:80 --name avilaplatforms-frontend avilaplatforms-frontend:latest"
echo ""
