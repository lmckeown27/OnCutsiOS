...
export function saveOfflineAction(action: string, data: any): void {
  try {
    ...
    localStorage.setItem('avilaplatforms_offline_actions', JSON.stringify(offlineActions));
  } catch (error) {
...
export function getOfflineActions(): Array<{
  ...
  try {
    const actions = localStorage.getItem('avilaplatforms_offline_actions');
...
export function clearOfflineActions(): void {
  try {
    localStorage.removeItem('avilaplatforms_offline_actions');
  } catch (error) {
