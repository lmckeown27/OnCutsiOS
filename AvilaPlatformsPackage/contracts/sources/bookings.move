/// AvilaPlatforms Bookings Module
/// 
...
module avila_platforms::bookings {
    use std::signer;
    ...
    public entry fun create_booking(
        ...
        // Create booking
        let registry = borrow_global_mut<BookingRegistry>(@avila_platforms);
    ...
    public fun get_booking_status(booking_id: u64): u8 acquires BookingRegistry {
        let registry = borrow_global<BookingRegistry>(@avila_platforms);
        assert!(table::contains(&registry.bookings, booking_id), E_BOOKING_NOT_FOUND);
    ...
    public fun get_total_bookings(): u64 acquires BookingRegistry {
        borrow_global<BookingRegistry>(@avila_platforms).total_bookings
    }
