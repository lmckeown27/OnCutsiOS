/// AvilaPlatforms Platform Admin Module
/// 
...
module avila_platforms::platform_admin {
    use std::signer;
    ...
    use avila_platforms::user_accounts;
    use avila_platforms::bookings;
    ...
    public entry fun add_admin(
        ...
        let config = borrow_global_mut<PlatformConfig>(@avila_platforms);
    ...
    public entry fun file_dispute(
        ...
        let registry = borrow_global_mut<DisputeRegistry>(@avila_platforms);
        let caller_addr = signer::address_of(caller);
    ...
    public fun assert_not_paused() acquires PlatformConfig {
        let config = borrow_global<PlatformConfig>(@avila_platforms);
    ...
    public fun is_paused(): bool acquires PlatformConfig {
        borrow_global<PlatformConfig>(@avila_platforms).is_paused
    }

    #[view]
    public fun get_accumulated_fees(): u64 acquires PlatformConfig {
        borrow_global<PlatformConfig>(@avila_platforms).accumulated_fees
    }
    ...
    public fun get_open_disputes_count(): u64 acquires DisputeRegistry {
        vector::length(&borrow_global<DisputeRegistry>(@avila_platforms).open_disputes)
    }
}
