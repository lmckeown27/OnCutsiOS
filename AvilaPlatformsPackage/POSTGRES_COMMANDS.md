# AvilaPlatforms PostgreSQL Commands Reference
...
```bash
sudo -u postgres psql -d avilaplatforms
```
...
AvilaPlatforms uses a role-based permission system with the following hierarchy:
...
## Number of Users
'''bash
sudo -u postgres psql -d avilaplatforms -c "SELECT COUNT(*) AS total_users FROM users;"
'''

### Number of Users (By Role)
'''bash
sudo -u postgres psql -d avilaplatforms -c "SELECT role, COUNT(*) FROM users GROUP BY role ORDER BY COUNT(*) DESC;"
'''

### View All Users (Table Format)
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT email, first_name, last_name, role, email_verified FROM users;"
```

### View All Users (Expanded/Vertical Format)
```bash
sudo -u postgres psql -d avilaplatforms -x -c "SELECT id, email, first_name, last_name, role, email_verified, \"createdAt\" FROM users;"
```

### View All Users (Formatted Table)
```bash
sudo -u postgres psql -d avilaplatforms -c "
SELECT 
...
sudo -u postgres psql -d avilaplatforms -x -c "SELECT * FROM users WHERE email = 'EMAIL';"

# Example:
sudo -u postgres psql -d avilaplatforms -x -c "SELECT * FROM users WHERE email = 'liam.mckeown38415@gmail.com';"
```

### View User with All Details
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id, email, first_name, last_name, \"displayName\", role, email_verified, \"isVerified\", \"avatarUrl\", \"instagramHandle\", \"createdAt\" FROM users WHERE email = 'user@example.com';"
```
...
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET role = 'BARBER', \"updatedAt\" = CURRENT_TIMESTAMP WHERE email = 'user@example.com';"

# Step 2: Activate barber record (if exists) or create one
# Check if barber record exists first:
sudo -u postgres psql -d avilaplatforms -c "SELECT b.id, b.\"isActive\" FROM barbers b JOIN users u ON b.\"userId\" = u.id WHERE u.email = 'user@example.com';"

# If barber record exists, activate it:
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isActive\" = true, \"updatedAt\" = CURRENT_TIMESTAMP WHERE \"userId\" = (SELECT id FROM users WHERE email = 'user@example.com');"

# ============================================================================
# PROMOTE TO CAMPUS_MANAGER
# ============================================================================
# Step 1: Update user role AND set their campusId
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET role = 'CAMPUS_MANAGER', \"campusId\" = 'CAMPUS_UUID_HERE', \"updatedAt\" = CURRENT_TIMESTAMP WHERE email = 'user@example.com';"

# Step 2: Set isCampusManager flag on barber record
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isCampusManager\" = true, \"isActive\" = true, \"updatedAt\" = CURRENT_TIMESTAMP WHERE \"userId\" = (SELECT id FROM users WHERE email = 'user@example.com');"

# ============================================================================
# DEMOTE TO CONSUMER (from BARBER or CAMPUS_MANAGER)
# ============================================================================
# Step 1: Update user role
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET role = 'CONSUMER', \"updatedAt\" = CURRENT_TIMESTAMP WHERE email = 'user@example.com';"

# Step 2: Deactivate barber record and remove campus manager flag
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isActive\" = false, \"isCampusManager\" = false, \"updatedAt\" = CURRENT_TIMESTAMP WHERE \"userId\" = (SELECT id FROM users WHERE email = 'user@example.com');"

# Step 3: Delete any CM-barber direct conversations (optional cleanup)
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM messages WHERE conversation_id IN (SELECT id FROM conversations WHERE booking_id IS NULL AND (user1_id = (SELECT id FROM users WHERE email = 'user@example.com') OR user2_id = (SELECT id FROM users WHERE email = 'user@example.com'))); DELETE FROM conversations WHERE booking_id IS NULL AND (user1_id = (SELECT id FROM users WHERE email = 'user@example.com') OR user2_id = (SELECT id FROM users WHERE email = 'user@example.com'));"

# ============================================================================
# PROMOTE TO ADMIN
# ============================================================================
# Admins have campus manager privileges at ALL campuses automatically
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET role = 'ADMIN', \"updatedAt\" = CURRENT_TIMESTAMP WHERE email = 'user@example.com';"
...
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isCampusManager\" = false, \"updatedAt\" = CURRENT_TIMESTAMP WHERE \"userId\" = (SELECT id FROM users WHERE email = 'user@example.com');"

# ============================================================================
# QUICK REFERENCE: Get Campus IDs
# ============================================================================
sudo -u postgres psql -d avilaplatforms -c "SELECT id, name FROM campuses WHERE name ILIKE '%cal poly%' OR name ILIKE '%your campus%' LIMIT 10;"
```

### Update User Email Verification
```bash
# Verify email
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET email_verified = true WHERE email = 'user@example.com';"

# Unverify email
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET email_verified = false WHERE email = 'user@example.com';"
```

### Update User Profile
```bash
# Update display name
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET \"displayName\" = 'New Name' WHERE email = 'user@example.com';"

# Update avatar
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET \"avatarUrl\" = 'https://example.com/image.jpg' WHERE email = 'user@example.com';"

# Update Instagram
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET \"instagramHandle\" = '@username' WHERE email = 'user@example.com';"
```

### Block/Unblock User
```bash
# Block user
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET \"isBlocked\" = true WHERE email = 'user@example.com';"

# Unblock user
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET \"isBlocked\" = false WHERE email = 'user@example.com';"
```

### Delete User (Simple)
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM users WHERE email = 'user@example.com';"
```
...
EMAIL='user@example.com' && sudo -u postgres psql -d avilaplatforms -c "
BEGIN;
...
sudo -u postgres psql -d avilaplatforms -c "SELECT id, email, first_name, role FROM users WHERE email = 'test@example.com';"

# Step 2: Delete all related records (replace USER_ID with actual UUID)
# Delete in order to avoid foreign key constraints

# Delete notifications
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM notifications WHERE \"userId\" = 'USER_ID';"

# Delete bookings (as consumer or barber)
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM bookings WHERE \"consumerId\" = 'USER_ID' OR \"barberId\" = 'USER_ID';"

# Delete messages
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM messages WHERE \"senderId\" = 'USER_ID' OR \"receiverId\" = 'USER_ID';"

# Delete conversations
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM conversations WHERE \"consumerId\" = 'USER_ID' OR \"barberId\" = 'USER_ID';"

# Delete barber application (if any)
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM barber_applications WHERE \"userId\" = 'USER_ID';"

# Delete barber profile (if any)
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM barbers WHERE \"userId\" = 'USER_ID';"

# Delete the user
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM users WHERE id = 'USER_ID';"
```
...
EMAIL='test@example.com' && sudo -u postgres psql -d avilaplatforms -c "
DO \$\$
DECLARE uid UUID;
BEGIN
...
### Count Users by Role
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT role, COUNT(*) FROM users GROUP BY role;"
```

### Find User ID by Email
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id FROM users WHERE email = 'user@example.com';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -x -c "
SELECT 
  u.id,
  u.first_name,
  u.last_name,
...
### Count Consumers
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT COUNT(*) AS total_consumers FROM users WHERE role = 'CONSUMER';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT b.*, u.role, u.first_name, u.last_name FROM barbers b JOIN users u ON b.\"userId\" = u.id WHERE u.email = 'barber@example.com';"
```

### View Barber Schedule
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT u.email, b.\"weeklySchedule\" FROM barbers b JOIN users u ON b.\"userId\" = u.id WHERE u.email = 'barber@example.com';"
```

### View Barber Specialties
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT u.email, b.specialties FROM barbers b JOIN users u ON b.\"userId\" = u.id WHERE u.email = 'barber@example.com';"
```

### Update Barber Bio
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET bio = 'New bio text here' FROM users WHERE barbers.\"userId\" = users.id AND users.email = 'barber@example.com';"
```

### Activate/Deactivate Barber
```bash
# Deactivate
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isActive\" = false FROM users WHERE barbers.\"userId\" = users.id AND users.email = 'barber@example.com';"

# Activate
sudo -u postgres psql -d avilaplatforms -c "UPDATE barbers SET \"isActive\" = true FROM users WHERE barbers.\"userId\" = users.id AND users.email = 'barber@example.com';"
```

### Delete Barber Profile
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM barbers USING users WHERE barbers.\"userId\" = users.id AND users.email = 'barber@example.com';"
```

### Describe Barbers Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d barbers"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT ba.id, u.email, u.first_name, ba.status, ba.years_experience, ba.created_at FROM barber_applications ba JOIN users u ON ba.user_id = u.id ORDER BY ba.created_at DESC;"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT ba.*, u.email FROM barber_applications ba JOIN users u ON ba.user_id = u.id WHERE ba.status = 'pending';"
```
...
### Approve Application
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE barber_applications SET status = 'approved', reviewed_at = NOW() WHERE id = 'UUID_HERE';"
```

### Reject Application
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE barber_applications SET status = 'rejected', reviewed_at = NOW(), review_notes = 'Reason here' WHERE id = 'UUID_HERE';"
```

### Delete Application
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM barber_applications WHERE id = 'UUID_HERE';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d barber_applications"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE guest_barber_applications SET status = 'approved', reviewed_at = NOW() WHERE id = 'UUID_HERE';"
```

### Reject Guest Application
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE guest_barber_applications SET status = 'rejected', reviewed_at = NOW(), review_notes = 'Reason here' WHERE id = 'UUID_HERE';"
```

### Delete Guest Application
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM guest_barber_applications WHERE id = 'UUID_HERE';"
```

### Describe Guest Barber Applications Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d guest_barber_applications"
```
...
### View All Campuses
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id, name, city, state, \"isActive\" FROM campuses ORDER BY name;"
```
...
### View Campus Count
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT COUNT(*) as total_campuses FROM campuses;"
```

### Find Campus by Name
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM campuses WHERE name ILIKE '%poly%';"
```

### Find Campuses by State
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT name, city FROM campuses WHERE state = 'CA' ORDER BY name;"
```
...
sudo -u postgres psql -d avilaplatforms -c "UPDATE campuses SET \"isActive\" = false WHERE slug = 'campus-slug';"

# Update pricing
sudo -u postgres psql -d avilaplatforms -c "UPDATE campuses SET \"basePriceUsdCents\" = 2500, \"averagePriceUsdCents\" = 4000 WHERE slug = 'campus-slug';"
```

### Delete Invalid Campuses
```bash
# Delete non-university entries (like GMAIL, ICLOUD)
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM campuses WHERE name IN ('GMAIL', 'ICLOUD');"
```
...
cat ~/AvilaPlatforms/backend/src/database/seed_campuses.sql | sudo -u postgres psql -d avilaplatforms

# Option 2: Copy to tmp first
cp ~/AvilaPlatforms/backend/src/database/seed_campuses.sql /tmp/
sudo -u postgres psql -d avilaplatforms -f /tmp/seed_campuses.sql

# Option 3: Fix permissions then run directly
chmod 644 ~/AvilaPlatforms/backend/src/database/seed_campuses.sql
sudo -u postgres psql -d avilaplatforms -f ~/AvilaPlatforms/backend/src/database/seed_campuses.sql
```

### Describe Campuses Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d campuses"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d locations"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d campus_locations"
sudo -u postgres psql -d avilaplatforms -c "\d barber_locations"
```
...
### View Open Slots
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM availability WHERE status = 'OPEN' ORDER BY \"startTime\";"
```

### View Booked Slots
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM availability WHERE status = 'BOOKED' ORDER BY \"startTime\" DESC;"
```

### Describe Availability Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d availability"
```
...
### View All Bookings
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id, \"barberId\", \"consumerId\", \"serviceType\", status, \"requestedAt\" FROM bookings ORDER BY \"requestedAt\" DESC LIMIT 20;"
```

### View All Bookings (Expanded)
```bash
sudo -u postgres psql -d avilaplatforms -x -c "SELECT * FROM bookings ORDER BY \"requestedAt\" DESC LIMIT 10;"
```

### View Bookings by Status
```bash
# Status values: PENDING, ACCEPTED, REJECTED, COMPLETED, CANCELLED
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM bookings WHERE status = 'PENDING';"
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM bookings WHERE status = 'ACCEPTED';"
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM bookings WHERE status = 'REJECTED';"
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM bookings WHERE status = 'COMPLETED';"
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM bookings WHERE status = 'CANCELLED';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE bookings SET status = 'ACCEPTED', \"acceptedAt\" = NOW() WHERE id = 'UUID_HERE';"
sudo -u postgres psql -d avilaplatforms -c "UPDATE bookings SET status = 'COMPLETED', \"completedAt\" = NOW() WHERE id = 'UUID_HERE';"
sudo -u postgres psql -d avilaplatforms -c "UPDATE bookings SET status = 'CANCELLED', \"cancelledAt\" = NOW() WHERE id = 'UUID_HERE';"
```

### Delete Booking
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM bookings WHERE id = 'UUID_HERE';"
```

### View BookingStatus Enum Values
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT unnest(enum_range(NULL::\"BookingStatus\"));"
```

### Describe Bookings Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d bookings"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id, user1_id, user2_id, service_name, booking_status, created_at, last_message_at FROM conversations ORDER BY last_message_at DESC;"
```

### View Conversations for a User
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM conversations WHERE user1_id = 'UUID_HERE' OR user2_id = 'UUID_HERE';"
```

### Delete Conversation
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM conversations WHERE id = 123;"
```

### Describe Conversations Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d conversations"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT id, conversation_id, content, message_type, is_read, created_at FROM messages ORDER BY created_at DESC LIMIT 50;"
```

### View Messages in a Conversation
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT m.id, u.email as sender, m.content, m.is_read, m.created_at FROM messages m JOIN users u ON m.sender_id = u.id WHERE m.conversation_id = 123 ORDER BY m.created_at;"
```

### Mark Messages as Read
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE messages SET is_read = true WHERE conversation_id = 123;"
```

### Delete Message
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM messages WHERE id = 123;"
```

### Describe Messages Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d messages"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT email, first_name, last_name, role, location_permission FROM users WHERE location_permission = 'granted';"
```

### View Users Who Denied Location Permission
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT email, first_name, last_name, role, location_permission FROM users WHERE location_permission = 'denied';"
```

### Update User Location (Manual)
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET latitude = 37.2395, longitude = -121.9251, location_updated_at = NOW(), location_permission = 'granted' WHERE email = 'user@example.com';"
```

### Clear User Location
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE users SET latitude = NULL, longitude = NULL, location_updated_at = NULL, location_permission = 'prompt' WHERE email = 'user@example.com';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d users" | grep -E "latitude|longitude|location"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM notifications WHERE user_id = 'UUID_HERE' ORDER BY created_at DESC;"
```

### Mark Notifications as Read
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE notifications SET is_read = true WHERE user_id = 'UUID_HERE';"
```

### Delete Old Notifications
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM notifications WHERE created_at < NOW() - INTERVAL '30 days';"
```

### Describe Notifications Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d notifications"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT stripe_account_id FROM users WHERE email = 'barber@example.com';"
```
...
> **Note:** AvilaPlatforms uses Stripe for all payments. Blockchain payment columns are deprecated.
...
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM payment_transactions WHERE status = 'succeeded' ORDER BY created_at DESC;"

# Pending transactions
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM payment_transactions WHERE status = 'pending' ORDER BY created_at DESC;"

# Failed transactions
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM payment_transactions WHERE status = 'failed' ORDER BY created_at DESC;"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d payment_transactions"
```
...
### View All Escrows
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM escrows ORDER BY created_at DESC LIMIT 20;"
```

### View Active (Held) Escrows
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM escrows WHERE status = 'held' ORDER BY created_at DESC;"
```

### View Escrow by Booking
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM escrows WHERE booking_id = 'BOOKING_ID_HERE';"
```

### Describe Escrows Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d escrows"
```
...
### View User Ledger
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM ledger_entries WHERE user_id = 'UUID_HERE' ORDER BY created_at DESC;"
```

### View Recent Ledger Entries
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM ledger_entries ORDER BY created_at DESC LIMIT 50;"
```

### Describe Ledger Entries Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d ledger_entries"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM withdrawal_requests ORDER BY requested_at DESC;"
```

### View Pending Withdrawals
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM withdrawal_requests WHERE status = 'pending';"
```

### Update Withdrawal Status
```bash
sudo -u postgres psql -d avilaplatforms -c "UPDATE withdrawal_requests SET status = 'completed', completed_at = NOW() WHERE id = 'UUID_HERE';"
```

### Describe Withdrawal Requests Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d withdrawal_requests"
```
...
### View Recent Events
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT event_type, COUNT(*) FROM analytics_events WHERE timestamp > NOW() - INTERVAL '7 days' GROUP BY event_type ORDER BY COUNT(*) DESC;"
```

### View User Activity
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM analytics_events WHERE user_id = 'UUID_HERE' ORDER BY timestamp DESC LIMIT 50;"
```

### Describe Analytics Events Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d analytics_events"
```
...
### View All Referrals
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM referrals;"
```

### Describe Referrals Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d referrals"
```
...
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM stripe_events WHERE event_type = 'payment_intent.succeeded' ORDER BY timestamp DESC LIMIT 10;"

# Payment failed events
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM stripe_events WHERE event_type = 'payment_intent.payment_failed' ORDER BY timestamp DESC LIMIT 10;"

# Payout events
sudo -u postgres psql -d avilaplatforms -c "SELECT * FROM stripe_events WHERE event_type LIKE 'payout.%' ORDER BY timestamp DESC LIMIT 10;"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "\d stripe_events"
```
...
### List All Tables
```bash
sudo -u postgres psql -d avilaplatforms -c "\dt"
```

### Describe Any Table
```bash
sudo -u postgres psql -d avilaplatforms -c "\d TABLE_NAME"
```
...
### Check Database Size
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT pg_size_pretty(pg_database_size('avilaplatforms'));"
```

### Check Table Sizes
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT relname as table, pg_size_pretty(pg_total_relation_size(relid)) as size FROM pg_catalog.pg_statio_user_tables ORDER BY pg_total_relation_size(relid) DESC;"
```

### View UserRole Enum Values
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT unnest(enum_range(NULL::\"UserRole\"));"
```
...
### Full Backup
```bash
sudo -u postgres pg_dump avilaplatforms > ~/avilaplatforms_backup_$(date +%Y%m%d_%H%M%S).sql
```

### Backup Specific Table
```bash
sudo -u postgres pg_dump -t users avilaplatforms > ~/users_backup.sql
sudo -u postgres pg_dump -t barbers avilaplatforms > ~/barbers_backup.sql
```

### Restore from Backup
```bash
sudo -u postgres psql -d avilaplatforms < ~/avilaplatforms_backup.sql
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "TRUNCATE TABLE messages CASCADE;"
sudo -u postgres psql -d avilaplatforms -c "TRUNCATE TABLE conversations CASCADE;"
```

### Drop Table Completely
```bash
sudo -u postgres psql -d avilaplatforms -c "DROP TABLE IF EXISTS table_name CASCADE;"
```

### Reset Auto-Increment Sequence
```bash
sudo -u postgres psql -d avilaplatforms -c "ALTER SEQUENCE conversations_id_seq RESTART WITH 1;"
sudo -u postgres psql -d avilaplatforms -c "ALTER SEQUENCE messages_id_seq RESTART WITH 1;"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "SELECT unnest(enum_range(NULL::\"ServiceType\"));"
```

### Add New ServiceType to Enum
```bash
# Example: Add TAPER to ServiceType enum
sudo -u postgres psql -d avilaplatforms -c "ALTER TYPE \"ServiceType\" ADD VALUE IF NOT EXISTS 'TAPER';"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM pending_registrations WHERE expires_at < NOW();"
```

### Clear All Pending Registrations (Testing Only)
```bash
sudo -u postgres psql -d avilaplatforms -c "DELETE FROM pending_registrations;"
```
...
```bash
sudo -u postgres psql -d avilaplatforms -c 'ALTER TABLE bookings ADD COLUMN IF NOT EXISTS "paymentRequestedAt" TIMESTAMPTZ;'
```
