...
AvilaPlatforms is a mobile-first web platform that connects college students with on-campus barbers. The platform streamlines the entire haircut booking experience—from discovering barbers at your university to scheduling appointments, messaging, and secure payment processing.

### What is AvilaPlatforms?

AvilaPlatforms solves a common problem on college campuses: finding reliable, affordable haircuts nearby. Instead of searching for off-campus barbershops or relying on word-of-mouth, students can browse verified barbers right at their university, view portfolios, check availability, and book appointments in minutes.
...
```bash
# Clone repository
git clone https://github.com/lmckeown27/AvilaPlatforms.git
cd AvilaPlatforms
...
# Start services
pm2 start backend/dist/index.js --name avilaplatforms-backend
```
...
CREATE DATABASE avilaplatforms;
CREATE USER avilaplatforms_user WITH PASSWORD 'your_secure_password';
GRANT ALL PRIVILEGES ON DATABASE avilaplatforms TO avilaplatforms_user;
\c avilaplatforms
GRANT ALL ON SCHEMA public TO avilaplatforms_user;
```
...
for f in src/database/migrations/*.sql; do
  sudo -u postgres psql -d avilaplatforms -f "$f"
done

# Seed campus data
sudo -u postgres psql -d avilaplatforms -f src/database/seed_campuses.sql
```
...
# Database
DATABASE_URL="postgresql://avilaplatforms_user:password@localhost:5432/avilaplatforms?schema=public"
...
EMAIL_FROM="AvilaPlatforms <noreply@avilaplatforms.com>"
...
# 2. Setup database
sudo -u postgres createdb avilaplatforms
...
sudo nano /etc/nginx/sites-available/avilaplatforms
...
```bash
# Check logs
pm2 logs avilaplatforms-backend --lines 50

# Rebuild
cd backend
rm -rf dist
npm run build
pm2 restart avilaplatforms-backend
```
...
```bash
# Test connection
sudo -u postgres psql -d avilaplatforms -c "SELECT 1;"
