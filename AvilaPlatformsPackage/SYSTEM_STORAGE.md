# AvilaPlatforms System Storage & Cleanup Reference
...
```bash
rm -rf ~/AvilaPlatforms/web-app/node_modules
rm -rf ~/AvilaPlatforms/backend/node_modules
```

### Reinstall Dependencies
```bash
cd ~/AvilaPlatforms/web-app && npm ci
cd ~/AvilaPlatforms/backend && npm ci
```
...
### Check Uploads Size
```bash
du -sh ~/AvilaPlatforms/backend/uploads
```

### List Upload Files
```bash
ls -lah ~/AvilaPlatforms/backend/uploads
```

### Count Upload Files
```bash
find ~/AvilaPlatforms/backend/uploads -type f | wc -l
```
...
```bash
du -sh ~/AvilaPlatforms/.git
```

### Clean Git (Remove Unreachable Objects)
```bash
cd ~/AvilaPlatforms && git gc --prune=now
```

### View Large Files in Git History
```bash
cd ~/AvilaPlatforms && git rev-list --objects --all | git cat-file --batch-check='%(objecttype) %(objectname) %(objectsize) %(rest)' | sed -n 's/^blob //p' | sort -rnk2 | head -20
```
...
```bash
/usr/local/bin/avilaplatforms-cleanup.sh
```

### View Cleanup Log
```bash
cat /var/log/avilaplatforms-cleanup.log
tail -50 /var/log/avilaplatforms-cleanup.log
```
...
```bash
sudo /usr/local/bin/avilaplatforms-cleanup.sh
```
