# Intera (consumer) — product, UI/UX, and AvilaPlatformsPackage integration

This document describes the **shipping Intera iOS app** today: purpose, major flows, UI/UX patterns, backend contracts, and how **`AvilaPlatformsPackage`** (`AvilaPlatformsModule` + `Core`) fits in. Use it as direction for a **Provider (barber) companion app** that shares the same **AvilaPlatforms** backend and optionally the same Swift packages.

---

## 1. What Intera is

**Intera** is a **consumer-facing** iOS app for **requesting and managing on-site services** (hair, nails, etc.) from **providers** listed on the **AvilaPlatforms** platform. It is not the barber dashboard; it is the **student / consumer** shell.

**Primary jobs-to-be-done**

1. **Discover** providers near the user (or full list), filter by service category, search by name.
2. **Book** a service (intake → confirmation) against the AvilaPlatforms **`bookings-simple`** API.
...
- API root is configured as **`https://campuscut.com/api/v1`** (see `AppConfiguration` in the Intera target). Same deployment as the AvilaPlatforms web app (`VITE_API_URL=/api/v1` pattern).
- Intera uses **JWT** (`Authorization: Bearer …`) from AvilaPlatforms auth (`/auth/apple`, `/auth/google`, email login, register).
...
| **`AvilaPlatformsModule`** (SPM) | Shared **models** (`ServiceProvider`), **auth helpers** (`AvilaPlatformsAuthTokenStore`, sign-up flows), **messaging** (`AvilaPlatformsChatManager`), **legal**, **ServiceType** filter enum, **live-data banner**, sample **AvilaPlatformsHomeView** / **BarberDashboardView** (shell demos—not the main Intera consumer UI). |
...
- `AvilaPlatformsSessionSync.appSessionManager` is set from `RootView` so package code that needs the shell session can reach **`AppSessionManager`** (see `CoreUserSessionProtocol` / `AvilaPlatformsSessionSync.swift`).
...
## 5. `AvilaPlatformsPackage` contents (what to reuse)

**Products** (`Package.swift`):

- **`AvilaPlatformsModule`** — no Stripe dependency; safe for UI + auth + models shared with a provider app if you avoid pulling Stripe into that target.
- **`Core`** — Stripe + checkout; link only on targets that pay out or take card.

**`AvilaPlatformsModule` highlights** (`ios-module/Sources/AvilaPlatformsModule/`)
...
- **Auth**: `AvilaPlatformsAuthService`, `AvilaPlatformsSignUpView`, `AvilaPlatformsSignUpAPI`, `AvilaPlatformsAuthTokenStore` (keychain-like token storage used with `AppSessionManager`).
- **Messaging**: `AvilaPlatformsChatManager` (Intera also uses bespoke `MessagingView` / `ChatViewModel` wired to same backend).
- **Legal**: `AvilaPlatformsLegal`.
- **Views**: `AvilaPlatformsHomeView`, `ConsumerHomeView`, `BarberDashboardView` — **reference / optional**; Intera’s real consumer shell is **`ScreensConsumerHome`** + hub.
...
For a **Provider app**, you may still need **`Core`** if providers collect **deposits** or **payouts** through the same Stripe Connect story; otherwise you might depend only on **`AvilaPlatformsModule`** plus your own networking.

---

## 6. How Intera wires `AvilaPlatformsModule` and `Core`

**Imports (representative)**

- `import AvilaPlatformsModule` — session, `ServiceType`, `ServiceProvider`, sign-up, legal, chat manager, live banner, `UserRole`-related types, Google/OAuth helpers where bridged.
...
| `RootView.swift` | Auth gating, role switch, `ChatViewModel`, payment/review full-screen covers, `AvilaPlatformsSessionSync`. |
...
| `AvilaPlatformsPackage/Package.swift` | SPM products and Stripe dependency on `Core`. |
...
2. **Reuse `AvilaPlatformsModule`** for **`ServiceType`**, tokens, optional sign-up/legal; consider reusing **`BarberDashboardViewModel`** patterns or replacing with native provider UX.
...
| Term | Meaning |
|------|---------|
| **AvilaPlatforms** | Backend + web brand; hosts `/api/v1` and Socket.IO. |
...
A **curated** route table, **`users.id` vs `barbers.id`**, **`GET /bookings-simple` role behavior**, **socket room + event catalog** (from `AvilaPlatformsPackage/backend` + Intera `MessagingRealtime.swift`), and **product checklists** live in:
