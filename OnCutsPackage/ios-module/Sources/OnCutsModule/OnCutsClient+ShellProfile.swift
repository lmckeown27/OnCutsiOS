...
public struct OnCutsProfileBooking: Sendable, Identifiable, Hashable {
...
public struct OnCutsBarberReviewSnippet: Sendable, Identifiable, Hashable {
    public let id: String
...
extension OnCutsClient {
    ...
    public func fetchMyBookings() async throws -> [OnCutsProfileBooking] {
    ...
    public func fetchBarberPortfolioURLs(barberId: String) async throws -> [URL] {
        ...
        return paths.compactMap { OnCutsS3ImageURL.url(forStoredPath: $0) }
    }
    ...
    public func fetchBarberReviewSnippets(barberId: String) async throws -> [OnCutsBarberReviewSnippet] {
        let trimmedId = barberId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedId.isEmpty else { return [] }
        let api = OnCutsAPIService(session: session, environment: environment)
    ...
    private static func mapBookingToProfile(_ b: Booking) -> OnCutsProfileBooking? {
    ...
    private static func mapReview(_ r: Review) -> OnCutsBarberReviewSnippet {
        ...
        return OnCutsBarberReviewSnippet(
            id: r.id,
            authorName: author,
            rating: r.rating,
            body: body,
