...
internal enum OnCutsAPIDecoding {
    ...
    static func decodePayload<T: Decodable>(_ type: T.Type, from data: Data, decoder: JSONDecoder) throws -> T {
        if data.isEmpty {
            throw OnCutsAPIError.decodingError(
        ...
        if let envelope = try? decoder.decode(APIEnvelope<T>.self, from: data) {
            if envelope.success == false {
                ...
                throw OnCutsAPIError.serverError(statusCode: 0, message: message)
            }
        ...
        do {
            ...
            throw OnCutsAPIError.decodingError(error)
        }
    }
}
