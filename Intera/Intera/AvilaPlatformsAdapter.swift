...
//  Intera
//
//  Adapter to convert AvilaPlatforms-specific models to unified ServiceProvider model
//
...
//
// NOTE: This extension will be enabled when the AvilaPlatforms package is integrated.
// The AvilaPlatforms package provides the `Barber` type that this adapter converts to `ServiceProvider`.
//
// To enable this when the package is added:
// 1. Uncomment the extension below
// 2. Import the AvilaPlatforms package at the top of this file
// 3. Verify the property names match (id, userId, fullName, bio, etc.)

/*
extension Barber {
    /// Converts a AvilaPlatforms Barber to the unified ServiceProvider model for UI display
    func toServiceProvider() -> ServiceProvider {
        ServiceProvider(
            ...
            bio: self.bio,
            instagramHandle: nil, // AvilaPlatforms Barber doesn't have this field
...
extension ServiceProvider {
    /// Mock service providers from AvilaPlatforms barbers
    static var avilaPlatformsMocks: [ServiceProvider] {
...
/*
 ...
 // When fetching from AvilaPlatforms API:
