...
// MARK: - Extended UserSession for CampusCuts

extension UserSession {
    
    /// Mock student user with full CampusCuts profile
...
// MARK: - CampusCuts Backend API Models

/// User profile from your backend
struct CampusCutsUserProfile: Codable {
    let userId: String
    let email: String
    ...
    static let mockStudent = CampusCutsUserProfile(
    ...
    static let mockBarber = CampusCutsUserProfile(
...
struct CampusCutsBooking: Codable, Identifiable {
    let id: String
    ...
    static let mockUpcoming = CampusCutsBooking(
        id: "book_1234567890",
...
extension CampusCutsUserProfile {
...
final class MockCampusCutsAPI {
    ...
    static func login(email: String, password: String) async throws -> UserSession {
        ...
        let profile = email.contains("barber")
            ? CampusCutsUserProfile.mockBarber
            : CampusCutsUserProfile.mockStudent
    ...
    static func fetchUserProfile(userId: String) async throws -> CampusCutsUserProfile {
        try await Task.sleep(for: .seconds(0.5))
        return CampusCutsUserProfile.mockStudent
    }
    ...
    static func createBooking(
        ...
        serviceType: String,
        scheduledAt: Date
    ) async throws -> CampusCutsBooking {
        try await Task.sleep(for: .seconds(1))
        
        return CampusCutsBooking(
...
/*
 ...
 let session = try await MockCampusCutsAPI.login(
     email: "alex.johnson@university.edu",
 ...
 let profile = try await MockCampusCutsAPI.fetchUserProfile(
     userId: session.userId
 )
 
 // Example: Create booking
 let booking = try await MockCampusCutsAPI.createBooking(
     barberId: "barber-123",
     serviceType: "Haircut",
 ...
 if let stripeId = session.stripeCustomerId {
     let customer = try await MockCampusCutsAPI.fetchStripeCustomer(
         customerId: stripeId
     )
 }
