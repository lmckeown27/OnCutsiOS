# AvilaPlatforms Module Integration Guide

## Overview

This guide explains how to integrate the **AvilaPlatforms Feature Module** into your **Intera Shell** app.
...
└─────────────────────────────────────┘
              ↓
┌─────────────────────────────────────┐
│   AvilaPlatforms Feature Module         │
│   (Pulled as Package Dependency)    │
│  ┌──────────────────────────────┐   │
│  │  AvilaPlatformsModuleBuilder     │   │
│  │  .build(with: session)       │   │
│  └──────────────────────────────┘   │
└─────────────────────────────────────┘
```
...
### 1. Add the AvilaPlatforms Package

#### Option A: Remote Package (Production)
1. In Xcode: File → Add Package Dependencies
2. Enter repository URL: `https://github.com/YourOrg/AvilaPlatforms.git`
3. Select version/branch
4. Add to Intera target

#### Option B: Local Package (Development)
1. Clone AvilaPlatforms repo locally
...
git commit -m "Add AvilaPlatforms dependency"

# When working on AvilaPlatforms locally:
cd /path/to/AvilaPlatforms
# Make changes
git commit -m "Update feature"

# Changes appear in Intera immediately (Xcode uses local override)
# When ready, push AvilaPlatforms changes:
...
The **AvilaPlatforms module** expects a session object that conforms to `UserSessionProtocol`.
...
```swift
import AvilaPlatforms // Import the module

// Inside MainCoordinator class:

/// Navigate to the AvilaPlatforms marketplace
func showAvilaPlatformsMarketplace() {
    guard let session = sessionManager.currentSession else { return }
    
    // Use the module's builder to create the view
    let marketplaceView = AvilaPlatformsModuleBuilder.buildMarketplace(with: session)
...
/// Handle deep links into AvilaPlatforms features
func handleAvilaPlatformsDeepLink(bookingId: String) {
    guard let session = sessionManager.currentSession else { return }
    
    let bookingView = AvilaPlatformsModuleBuilder.buildBookingDetail(
        bookingId: bookingId,
        session: session
    )
...
Replace the placeholder marketplace view with the real AvilaPlatforms module:

```swift
// In UIMainTabView.swift

import AvilaPlatforms
...
private func viewForTab(_ tab: AppTab) -> some View {
    ...
    case .marketplace:
        ...
        if let session = sessionManager.currentSession {
            AvilaPlatformsModuleBuilder.buildMarketplace(with: session)
        } else {
...
If the AvilaPlatforms module needs to communicate back to the Shell (e.g., "user logged out from settings"), create a delegate:

```swift
// In your AvilaPlatforms integration
extension MainCoordinator: AvilaPlatformsModuleDelegate {
    func avilaPlatformsDidRequestLogout() {
        sessionManager.logout()
        popToRoot()
    }
    
    func avilaPlatformsDidCompleteBooking(id: String) {
...
2. Add URL Scheme: `avilaplatforms`
...
func handleDeepLink(_ url: URL) {
    // Parse URL like: avilaplatforms://booking/abc123
    ...
    case "booking":
        if let bookingId = components?.path.components(separatedBy: "/").last {
            handleAvilaPlatformsDeepLink(bookingId: bookingId)
        }
...
@testable import AvilaPlatforms

@Suite("AvilaPlatforms Integration Tests")
struct AvilaPlatformsIntegrationTests {
    ...
    func testModuleBuilding() async throws {
        ...
        let view = AvilaPlatformsModuleBuilder.buildMarketplace(with: session)
...
4. Verify AvilaPlatforms content loads
5. Test deep link: `avilaplatforms://booking/test123`
...
### "Cannot find 'AvilaPlatformsModuleBuilder' in scope"
- Add `import AvilaPlatforms` at the top of the file
...
1. **Pull the AvilaPlatforms repository** and add it as a package
