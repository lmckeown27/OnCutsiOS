...
// MARK: - Extended UserSession for AvilaPlatforms

extension UserSession {
    
    /// Mock student user with full AvilaPlatforms profile
    ...
    static let mockBarberComplete = UserSession(
        ...
        email: "jordan.smith@avilaplatforms.com",
        displayName: "Jordan Smith",
        role: .barber,
        stripeCustomerId: "cus_XYZaB987654321", // Barbers are also Stripe customers
        profileImageURL: "https://avilaplatforms-assets.s3.amazonaws.com/profiles/jordan.jpg",
    ...
    static let mockAdmin = UserSession(
        ...
        email: "admin@avilaplatforms.com",
...
// MARK: - AvilaPlatforms Backend API Models

/// User profile from your backend
struct AvilaPlatformsUserProfile: Codable {
    let userId: String
    let email: String
    ...
    static let mockStudent = AvilaPlatformsUserProfile(
    ...
    static let mockBarber = AvilaPlatformsUserProfile(
...
struct AvilaPlatformsBooking: Codable, Identifiable {
    let id: String
    ...
    static let mockUpcoming = AvilaPlatformsBooking(
        id: "book_1234567890",
...
extension AvilaPlatformsUserProfile {
...
final class MockAvilaPlatformsAPI {
    ...
    static func login(email: String, password: String) async throws -> UserSession {
        ...
        let profile = email.contains("barber")
            ? AvilaPlatformsUserProfile.mockBarber
            : AvilaPlatformsUserProfile.mockStudent
    ...
    static func fetchUserProfile(userId: String) async throws -> AvilaPlatformsUserProfile {
        try await Task.sleep(for: .seconds(0.5))
        return AvilaPlatformsUserProfile.mockStudent
    }
    ...
    static func createBooking(
        ...
        serviceType: String,
        scheduledAt: Date
    ) async throws -> AvilaPlatformsBooking {
        try await Task.sleep(for: .seconds(1))
        
        return AvilaPlatformsBooking(
...
/*
 ...
 let session = try await MockAvilaPlatformsAPI.login(
     email: "alex.johnson@university.edu",
 ...
 let profile = try await MockAvilaPlatformsAPI.fetchUserProfile(
     userId: session.userId
 )
 
 // Example: Create booking
 let booking = try await MockAvilaPlatformsAPI.createBooking(
     barberId: "barber-123",
     serviceType: "Haircut",
 ...
 if let stripeId = session.stripeCustomerId {
     let customer = try await MockAvilaPlatformsAPI.fetchStripeCustomer(
         customerId: stripeId
     )
 }
