//
//  CampusCutsReviewModels.swift
//  Intera
//
//  Decodes CampusCuts `Review` payloads (barber list embed + GET /barbers/:id/reviews).
//
...
struct CampusCutsReviewDTO: Decodable, Sendable {
    let id: String?
...
struct CampusCutsBarberReviewsListResponse: Decodable, Sendable {
    let success: Bool?
    let data: [CampusCutsReviewDTO]?
}

// MARK: - Map → UI model

extension CampusCutsReviewDTO {
...
extension Array where Element == CampusCutsReviewDTO {
