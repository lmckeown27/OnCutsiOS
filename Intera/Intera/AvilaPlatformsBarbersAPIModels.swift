//
//  AvilaPlatformsBarbersAPIModels.swift
//  Intera
//
...
private struct AvilaPlatformsBarbersResponse: Decodable, Sendable {
    let success: Bool?
    let data: [AvilaPlatformsBarberDTO]?
}
...
private struct AvilaPlatformsBarberDTO: Decodable, Sendable {
    ...
    let pricing: [AvilaPlatformsPricingDTO]?
    ...
    let serviceLocations: [AvilaPlatformsServiceLocationDTO]?
...
private struct AvilaPlatformsPricingDTO: Decodable, Sendable {
    let id: String?
    let name: String
...
private struct AvilaPlatformsServiceLocationDTO: Decodable, Sendable {
    let id: String?
    let name: String?
}
...
private struct AvilaPlatformsBarberDetailResponse: Decodable, Sendable {
    let success: Bool?
    let data: AvilaPlatformsBarberDTO?
}

enum AvilaPlatformsBarbersDecoder {
    static func decodeServiceProviders(from data: Data) throws -> [ServiceProvider] {
        let decoder = JSONDecoder()
        decoder.keyDecodingStrategy = .convertFromSnakeCase

        let response = try decoder.decode(AvilaPlatformsBarbersResponse.self, from: data)
    ...
    static func decodeSingleServiceProvider(from data: Data) throws -> ServiceProvider {
        ...
        if let envelope = try? decoder.decode(AvilaPlatformsBarberDetailResponse.self, from: data),
...
enum AvilaPlatformsBarberDetailAPI {
    static func fetchServiceProvider(barberId: String, bearerToken: String?) async throws -> ServiceProvider {
        ...
        guard !trimmed.isEmpty else {
            throw NSError(
                domain: "AvilaPlatformsBarberDetailAPI",
                code: 400,
                userInfo: [NSLocalizedDescriptionKey: "Missing barber id"]
            )
        }
        guard let url = AppConfiguration.urlAvilaPlatformsBarber(barberId: trimmed) else {
            throw NSError(
                domain: "AvilaPlatformsBarberDetailAPI",
                code: 400,
        ...
        return try AvilaPlatformsBarbersDecoder.decodeSingleServiceProvider(from: data)
    }
}

private extension AvilaPlatformsBarberDTO {
